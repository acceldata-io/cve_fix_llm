#!/usr/bin/env python3
"""spark3_3_3_3 Medium+ CVE bumps + exceptions.

Owned bumps (compile-gate then PR + close):
  commons-lang3     3.13.0 -> 3.18.0   (OSV-23935)
  aircompressor     2.0.2  -> 2.0.3    (OSV-23930)
  commons-io        2.11.0 -> 2.16.1   (OSV-23963)
  commons-compress  1.21   -> 1.26.1   (OSV-24000, OSV-24001)
  okio              1.14.0 -> 1.17.6   (OSV-23927, inject DM)

Exceptions (Deferred): iceberg shaded, hadoop-client-runtime, parquet-jackson,
  hive 2.3 fork, guava 14 pin, protobuf 2.5.0 pin, jdom2 (Aliyun), tomcat-embed
  8.5 EOL (no public same-major fix), Spark-own CVEs needing 3.4+/3.5+ upgrade.

LOW To Dos left untouched.
...
RESULT = Path("/tmp/spark3_333_libs_compile_matrix.json")
...
EXCEPTIONS = [
    {
        "keys": ["OSV-23937", "OSV-23938"],
        "details": (
            "netty-common 4.1.97.Final is shaded inside iceberg-spark-runtime-"
            "3.3_2.12-1.4.3.jar, not Spark's managed netty.version. Fix requires "
            "upgrading Apache Iceberg. Exception Request (Deferred)."
        ),
    },
    {
        "keys": ["OSV-23944", "OSV-23945"],
        "details": (
            "parquet-avro 1.13.1 is shaded inside iceberg-spark-runtime-"
            "3.3_2.12-1.4.3.jar. Spark's own parquet.version pin cannot rewrite "
            "Iceberg relocated classes. Fix requires upgrading Apache Iceberg. "
            "Exception Request (Deferred)."
        ),
    },
    {
        "keys": ["OSV-23959", "OSV-23960"],
        "details": (
            "aircompressor 0.25 is shaded inside iceberg-spark-runtime-"
            "3.3_2.12-1.4.3.jar. Spark's standalone aircompressor pin cannot "
            "replace classes inside Iceberg. Fix requires upgrading Apache "
            "Iceberg. Exception Request (Deferred)."
        ),
    },
    {
        "keys": ["OSV-23932", "OSV-23933"],
        "details": (
            "avro 1.11.1 is shaded inside iceberg-spark-runtime-3.3_2.12-1.4.3.jar. "
    ...
    },
    {
        "keys": ["OSV-15480"],
        "details": (
            "jackson-databind 2.11.4 is shaded/relocated inside "
            "parquet-jackson-1.12.2.jar. A Spark Jackson bump cannot rewrite "
            "Parquet relocated classes. Fix requires upgrading Apache Parquet. "
            "Exception Request (Deferred)."
        ),
    },
    {
        "keys": ["OSV-23926", "OSV-23954"],
        "details": (
            "hive-exec / hive-llap-common 2.3.103 come from Spark's bundled Hive "
            "2.3.x fork (hive.version=2.3.103). CVE fixes land in Hive 4.0.x; "
            "upgrading the Hive fork is a major cross-component ODP change. "
            "Exception Request (Deferred)."
        ),
    },
    {
        "keys": ["OSV-24005", "OSV-24006"],
        "details": (
    ...
    {
        "keys": ["OSV-23941", "OSV-23942", "OSV-23943"],
        "details": (
            "protobuf-java 2.5.0 is pinned for Hadoop/Hive wire-format "
            "compatibility (protobuf.version=2.5.0). Upgrading to 3.16.1+/3.25.5 "
            "is a major change required across ODP components. Exception Request "
            "(Deferred)."
        ),
    },
    {
        "keys": ["OSV-12202"],
        "details": (
            "jdom2 is a compile dependency of the Aliyun OSS Java SDK, pulled in "
            "by Hadoop's Aliyun connector; it is a third-party transitive "
            "dependency and deferred. Exception Request (Deferred)."
        ),
    },
    {
        "keys": [
        ...
        "details": (
            "tomcat-embed-core is pinned at 8.5.99 (tomcat-embed.version) to "
            "override transitive libthrift Tomcat. Tomcat 8.5 is EOL (final "
            "8.5.100) and these CVEs have no public same-major fix; supported "
            "fixes are Tomcat 9.0.x/10.1.x/11.x, which is a breaking upgrade for "
            "Spark 3.3 thriftserver. Exception Request (Deferred)."
        ),
    },
    {
        "keys": ["OSV-23931"],
        "details": (
            "CVE-2024-23945 is in Spark's own spark-hive-thriftserver artifact; "
            "upstream fix is Spark 3.4.2+. Cannot be cleared by a third-party "
            "library bump on the 3.3.3 line. Exception Request (Deferred)."
        ),
    },
    {
        "keys": ["OSV-23980"],
        "details": (
            "CVE-2025-54920 is in Spark's own spark-core artifact; upstream fix "
            "is Spark 3.5.7+. Cannot be cleared by a third-party library bump on "
            "the 3.3.3 line. Exception Request (Deferred)."
        ),
    },
    {
        "keys": ["OSV-24007"],
        "details": (
            "CVE-2025-55039 is in Spark's own spark-network-common / yarn-shuffle "
            "artifact; upstream fix is Spark 3.4.4 / 3.5.2+. Cannot be cleared by "
            "a third-party library bump on the 3.3.3 line. Exception Request "
            "(Deferred)."
        ),
    },
]
...
def _set_prop(pom: Path, key: str, value: str) -> bool:
    ...
    # only replace first (root properties); avoid hadoop-2.7 profile override
...
def apply_okio(repo_dir: Path, version: str) -> list[str]:
    pom = repo_dir / "pom.xml"
    text = pom.read_text(encoding="utf-8", errors="replace")
    if re.search(r"<artifactId>okio</artifactId>\s*<version>", text):
        text2, n = re.subn(
            r"(<groupId>com\.squareup\.okio</groupId>\s*"
            r"<artifactId>okio</artifactId>\s*"
            r"<version>)([^<]+)(</version>)",
    ...
    block = (
        f"\n      <dependency>\n"
        f"        <groupId>com.squareup.okio</groupId>\n"
        f"        <artifactId>okio</artifactId>\n"
...
def apply_job(repo_dir: Path, job: dict) -> list[str]:
    if job["apply"] == "prop":
        if _set_prop(repo_dir / "pom.xml", job["prop"], job["version"]):
            return ["pom.xml"]
        return []
    if job["apply"] == "aircompressor":
        return apply_aircompressor(repo_dir, job["version"])
    if job["apply"] == "okio":
        return apply_okio(repo_dir, job["version"])
...
def try_compile(job: dict) -> dict:
    ...
    log = f"/tmp/spark3_333_build_{job['id']}_{job['version']}.log"
...
def deliver_all(results=None):
    ...
    for job in jobs:
        ...
        Path("/tmp/spark3_333_libs_deliver.json").write_text(
